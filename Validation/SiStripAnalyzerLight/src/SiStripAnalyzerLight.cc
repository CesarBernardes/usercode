// -*- C++ -*-
//
// Package:    SiStripAnalyzerLight
// Class:      SiStripAnalyzerLight
// 
/**\class SiStripAnalyzerLight SiStripAnalyzerLight.cc Validation/SiStripAnalyzer/src/SiStripAnalyzerLight.cc
...
class SiStripAnalyzerLight : public edm::EDAnalyzer {
   public:
      explicit SiStripAnalyzerLight(const edm::ParameterSet&);
      ~SiStripAnalyzerLight();
...
//
SiStripAnalyzerLight::SiStripAnalyzerLight(const edm::ParameterSet& iConfig)
...
SiStripAnalyzerLight::~SiStripAnalyzerLight()
{
...
void
SiStripAnalyzerLight::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for( ; DSViter != stripDigis->end(); DSViter++) {
	 ...
	 //Loop over Digis------------------------------------------
   	 ...
   	 for ( iter = begin ; iter != end; iter++ ) {  
	      ...
	      ++ModuleDigis;
	      DigisNt_->Fill(idH,idL, SubDet, detZ, detR, detPhi, detEta,strip,adc, APVn, bin);
	      
     }
          
    ModuleNt_->Fill(idH,idL, SubDet,  detZ, detR, detPhi, detEta, ModuleDigisNz, ModuleDigis, Nstrips, bin);
   
    for(unsigned int APVn =0; APVn< NAPVs; ++APVn){
   	   Double_t APVMedian = TMath::Median(128, ModuleDigisVect+128*APVn);
       CMNNt_->Fill(idH, idL, SubDet, NAPVs, APVn,APVMedian ,TotADCCharge[APVn], APVMultVect[APVn], bin);
    }  
   }   
...
void SiStripAnalyzerLight::beginJob()
{
  
  oFile_ = new TFile((const char*)outputFile_.c_str(), "RECREATE");
  
  ModuleNt_ = new TNtuple("TrackerModules", "TrackerModules","idH:idL:SubDet:z:R:phi:eta:MhitsNz:MHits:nstrips:b",1000);  //b is the multiplicity bin
  DigisNt_ = new TNtuple("TrackerDigis", "TrackerDigis","idH:idL:SubDet:z:R:phi:eta:strip:adc:APVn:b",1000);
  CMNNt_ = new TNtuple("TrackerCMN", "TrackerCMN", "idH:idL:SubDet:ModAPV:APVn:CMN:FullADC:APVMult:b", 1000);
 
}

// ------------ method called once each job just after ending the event loop  ------------
void 
SiStripAnalyzerLight::endJob() {
  oFile_->Write();
  oFile_->Close();
}

//define this as a plug-in
DEFINE_FWK_MODULE(SiStripAnalyzerLight);
